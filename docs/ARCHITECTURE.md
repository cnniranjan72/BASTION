...
- **Deployment targets (Phase 9, provisioned early)**: Neon (managed Postgres) and Render (hosting, via `render` CLI) credentials were provided and verified during Phase 1, stored in the gitignored root `.env` (never committed). Local dev and the test suite still run against Docker Compose Postgres, not Neon — Neon is reserved for the actual deployed environment once Phase 9 is reached, so the fast/free/isolated local loop (in particular the concurrency test in §8) isn't paying network latency or shared-instance contention on every run. **Verified live (post-launch)**: all 5 migrations applied cleanly against Neon, then a full signup → login → intercept → policy-block cycle (the Phase 8 demo scenario) ran successfully against it end to end — Neon is the real Postgres the Render deployment points at, not just a connection string that was checked once with `SELECT version()` and never exercised again.
- **Actually deployed to Render (post-launch)**: three Docker web services (interceptor, aggregator, frontend/nginx) plus a Key Value (Redis) instance, all in one Render project/environment. Real bugs found and fixed getting there — see decisions.md's Render-deployment entry for the full account: JWT keys now travel as env vars (no shared volume across separate Render services), and the frontend's nginx proxies to the backends' public HTTPS URLs rather than Render's private per-service networking, which never resolved through this container's DNS despite matching every documented requirement (region, workspace, and — after a full recreate — environment too).
